...
func (s *ResourceGenerator) makeFilterChainTerminatingGateway(
	...
	if cfgSnap.Kind == structs.ServiceKindTerminatingGateway {
		if apply, ok := cfgSnap.TerminatingGateway.ServiceEnvoyConfigApplications[service]; ok {
			if versions, ok := cfgSnap.TerminatingGateway.EnvoyConfigs[apply.EnvoyPatchSet.Name]; ok {
				if patch, ok := versions[apply.EnvoyPatchSet.Version]; ok {
					resolver, hasResolver := cfgSnap.TerminatingGateway.ServiceResolvers[service]
					if !hasResolver {
					...
					for i, patch := range patch.Patches {
						if patch.Type == structs.Replace {
							...
							fmt.Println("META", resolver.Meta)
							err = t.Execute(&raw, CustomFilterOptions{
								ClusterName: cluster,
								Meta:        resolver.Meta,
							})
							fmt.Println("OUT", raw.String())

							filter, err = makeTCPReplacementFilter(raw.String())

							if err != nil {
								s.Logger.Error("Sometimes bad things happen when making a filter", "error", err)
							}
						}
					}
				}
